...
public class Client {
    ...
    private void serveGet(List<String> request) throws IOException, ClassNotFoundException, WrongVersionNumber{                
        ...
        kryo.writeObject(kryo_output, name);        
        int verze;
        if(request.size()>=3){
        ...
        List<Byte> data = kryo.readObject(kryo_input, LinkedList.class);
        int c;
        for(byte b : data){
            c = (int)b + 128;
    ...
    private Database getDB() {
        for(int i = 0; i<attempt_limit; i++){
            try {                            
                ...
                kryo_output.flush();
                Database db = kryo.readObject(kryo_input, Database.class);
                return db;
            } catch (RuntimeException ex) {
                Logger.getLogger(Client.class.getName()).log(Level.SEVERE, null, ex);
            }            
     ...
     */
    private void serveAdd(List<String> request) throws IOException, NoSuchAlgorithmException {        
    ...
    //        ..zustane minimalni traffic, prida to casovou komplexitu pro server, snizi prostorovou kompl. na serveru            
        ...
        if (!alreadyPresent){                                    
            ...
            kryo.writeObject(kryo_output, target);
            kryo_output.flush();
        }                         
            ...
            while ((c=fin.read())!=-1){
                ...
                if(je_v_db){
                    ...
                    if(!nesparovane.isEmpty()){                        
                        ...
                        kryo.writeObject(kryo_output, nesparovane);                                       
                        nesparovane.clear();
                    }
